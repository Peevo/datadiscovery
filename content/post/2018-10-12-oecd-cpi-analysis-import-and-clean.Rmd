...
thumbnailImage: /img/OECD_logo_new.svg.pnd
categories:
  - R
tags:
  - Economics
...
Load in the data via rJSON package and convert to a dataframe.
...
```{r}
# change set current wd and change wd to our file source.  This is only required if the files are in another directory to your working.
path <- "C:/Users/squin/Desktop/Blog/Data"
setwd(path)

# Regex codes required for string matching. \\ escapes the match string as "." is a special character.  $ denotes the end of string.
...
We now have a dataset ready to go. Next post we'll do some analysis on the full data set.